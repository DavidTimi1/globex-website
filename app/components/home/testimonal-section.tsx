    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-16 text-center space-y-6">
          {/* Eyebrow with icon */}
          <div
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-50 border border-gray-200 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
          >
            <FaHeart className="text-gray-500 text-sm" />
            <span className="text-sm text-gray-500 font-medium">Testimonials</span>
          </div>

          {/* Title with word animation */}
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900">
            {['What', 'Our', 'Partners', 'Say'].map((word, i) => (
              <span
                key={i}
          ...
          {/* Subtitle */}
          <p
            className={`text-base md:text-lg text-gray-600 max-w-3xl mx-auto transition-all duration-700 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
          >
...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ testimonial, delay }) => {
      ...
      >
        ...
        <div className="relative h-full p-8 flex flex-col justify-between text-white">
          <div className="space-y-4">
            <FaQuoteLeft className="text-3xl text-white/90" />
  ...
  return (
    <div
      className="relative rounded-lg border border-gray-200 bg-gray-50 p-8 h-full min-h-[400px] flex flex-col justify-between overflow-hidden group hover:shadow-lg transition-all duration-500"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      style={{ transitionDelay: `${delay}ms` }}
    >
      ...
      <div className="relative z-10 space-y-6">
        <FaQuoteLeft className="text-3xl text-gray-900" />
        <p className="text-lg leading-relaxed text-gray-900">{testimonial.text}</p>
      </div>
